  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ++++                                     Python  高阶                                                                 ++++
  ++++  回调函数 设计模式  数据库MySQC  PySpark大数据   多线程  异步编程   网络编程 Soket   正则表达性 递归                ++++
  --------------------------------------------------------------------------------------------------------------------------
  ************************************   对象和类 魔术方法（构造方法） 详细说明   *****************************************
  ● 使用对象组织数据
  ● 在程序中可以做的设计，生产，填写 表格的组织形式的
    ● 在程序中设计表格 ，我们称之为: 设计类（clss）●
      clss student:
          name = None  # 记录学生姓名
  ...
  ● 魔术方法    
     ...
     ● Union类型：使用Union[类型，类型，...类型] 可以定义联合类型（多种类型）注解  多选一
          my_list:list[Union[str,int]] = [1,2,"it","heima"]  #列表元素是有可能是str 也有可能是int 二选一
   ...
   ● 闭包：在函数嵌套的前提下，内部函数使用了外部的变量，并且外部函数返回了内部函数，我们把这个使用外部函数变量的内部函数称之为闭包
    ...
    ● 装饰器：1 也是一种闭包，在不破坏目标函数原有代码和功能的前期下，为目标函数增加新功能
            ...
            @outer      #装饰器（增加功能） 把sleep() 传入outer(sleep) 函数运行   可以理解为 调用sleep 函数实际运行 outer(sleep)函数
            def sleep():            #睡眠函数
                import  random      #随机数函数
                import time         #时间函数
                print("睡眠中.........")
  ...
  ● 工厂模式：大量创建一个类的实例的时候，可以使用工厂模式
      ...
      ●  当发生修改，仅修改工厂类的创建方法即可，3符合现实世界的模式，即由工厂来制作产品（对象）
  ...
  ● SQL全称 Structured Query Language 结构化查询语言，用于访问和处理数据库的标准计算机语言；
   ...
   ● pytho & MySQL
    ...
    ++++                     ● PySpark  大数据 分布式计算                                      ++++
    ----------------------------------------------------------------------------------------------
  ● 安装pyspark 需安装JAVA 环境  写入文件需配置 Hadoop 依赖
  ...
  ● 语法： import threading  #导入线程模块
    ...
    ----------------------------------------------------------------------------------------------
      ● Python 的异步编程（Asynchronous Programming）是一种高效处理 I/O 密集型任务（如网络请求、文件读写等）的方式。
      ● 它允许程序在等待某些任务完成时，转而执行其他任务，从而提升程序的并发性能。
      ● 核心概念：
          ● 同步编程：程序按顺序执行，任务 A 完成后才能执行任务 B。
          ● 异步编程：程序启动任务后，无需等待其完成即可继续执行其他任务，任务完成时通过回调或事件通知结果。
          ● 协程（Coroutine）：异步编程的核心，是可暂停和恢复的函数，使用 async 和 await 定义。
          ● 事件循环（Event Loop）：负责调度和执行协程，管理任务的执行顺序。
          ● 任务（Task）：协程的封装，可被事件循环调度。
      1. 协程定义
        ● 语法：async def 函数名(参数):  使用 async 定义协程函数。 参数与普通函数一致，支持位置参数、关键字参数等。
          async def fetch_data(url, timeout=5):
              # 协程逻辑
              pass
      2. 运行协程   asyncio.run()：
        ● 语法：asyncio.run(协程对象)  参数：接收一个协程对象，启动事件循环并运行。
          import asyncio
          async def main():
              print("Hello")
          asyncio.run(main())
      3. 异步等待（await）
        ● 等待协程：await some_coroutine()
        ● 等待异步操作：如 await asyncio.sleep(1)
        ● 等待 Future 对象：如 await some_future
        ● 作用：暂停当前协程，等待异步操作完成。
      4. 并发执行 asyncio.gather()：
        ● 语法：await asyncio.gather(*协程列表) 参数：接收多个协程对象（可变参数）。作用：并发运行所有协程，返回结果列表。
                async def task(name):
                ...
                asyncio.create_task()：
        ● 语法：asyncio.create_task(协程对象) 参数：接收一个协程对象。作用：将协程包装为任务，立即调度到事件循环。
      5. 异步上下文管理器  async with：
        ● 语法：async with 表达式 as 变量: 参数： 表达式：返回异步上下文管理器（如 aiofiles.open()）。变量：接收上下文管理器实例。
        ● 作用：管理异步资源（如文件、网络连接）。
          import aiofiles
          async def read_file():
      ...
      6. 常见异步操作库
         ...
         ● aioredis：异步 Redis 客户端。
            aioredis.Redis()：
           ● 参数： host：Redis 主机地址（默认 'localhost'）。
      ...
      ● 参数总结表
             语法函数	                                 参数	                             说明
          async def                         	普通函数参数（位置、关键字、默认值）	  定义协程函数
          asyncio.run()	                      协程对象	                             启动事件循环并运行协程
          await	                              协程/异步操作/Future	                 等待异步操作完成
          asyncio.gather()	                  多个协程对象（可变参数）	              并发运行协程，返回结果列表
          asyncio.create_task()	              协程对象	                             将协程包装为任务
          async with	                        异步上下文管理器表达式	                 管理异步资源
          aiohttp.ClientSession	              connector,    timeout 等	              创建 HTTP 会话
          aioredis.Redis	                    host, port 等	                        连接 Redis
          asyncio.sleep()	                    delay（秒）                        	  模拟异步等待
      ● 总结
        ● 核心语法：async def、await、asyncio.run()。
        ● 参数类型：支持普通函数参数、协程对象、异步操作、上下文管理器等。
        ● 避免阻塞操作，使用异步库。
        ● 合理使用 asyncio.gather() 和 asyncio.create_task()。
        ● 捕获协程中的异常，确保程序健壮性。
    ----------------------------------------------------------------------------------------------
    ++++                                      网络编程 Soket                                   ++++
    ----------------------------------------------------------------------------------------------
